...
public class InAppWebView: WKWebView, UIScrollViewDelegate, WKUIDelegate,
    ...
    init(id: Any?, plugin: SwiftFlutterPlugin?, frame: CGRect, configuration: WKWebViewConfiguration,
        ...
        if let id = id, let registrar = plugin?.registrar {
            ...
            channelDelegate = WebViewChannelDelegate(webView: self, channel: channel)
        }
    ...
    private func gestureRecognizerWithDescriptionFragment(_ descriptionFragment: String) -> UILongPressGestureRecognizer? {
        let result = scrollView.subviews.compactMap({ $0.gestureRecognizers }).joined().first(where: {
        ...
        if let menu = contextMenu {
            if let menuItems = menu["menuItems"] as? [[String: Any]] {
                for menuItem in menuItems {
                    ...
                    let targetMethodName = "onContextMenuActionItemClicked-" + String(hash) + "-" +
                        (id is Int64 ? String(id as! Int64) : id as! String)
                    if !responds(to: Selector(targetMethodName)) {
                        ...
                        customIMPs.append(swizzledImplementation)
                    }
        ...
        if #available(iOS 16.0, *) {
            if let menu = contextMenu {
                ...
                if let contextMenuSettingsMap = menu["settings"] as? [String: Any?] {
                    _ = contextMenuSettings.parse(settings: contextMenuSettingsMap)
    ...
    public func prepare() {
        ...
        scrollView.addGestureRecognizer(recognizerForDisablingContextMenuOnLinks)
...
//        }
        ...
        if let settings = settings {
            ...
            if #available(iOS 11.0, *) {
                ...
                scrollView.contentInsetAdjustmentBehavior =
                    UIScrollView.ContentInsetAdjustmentBehavior(rawValue: settings.contentInsetAdjustmentBehavior)!
            }
    ...
    public static func preWKWebViewConfiguration(settings: InAppWebViewSettings?) -> WKWebViewConfiguration {
        ...
        if let settings = settings {
            ...
            configuration.selectionGranularity = WKSelectionGranularity(rawValue: settings.selectionGranularity)!
            ...
            if #available(iOS 10.0, *) {
                ...
                var dataDetectorTypes = WKDataDetectorTypes(rawValue: 0)
    ...
    @objc func onCreateContextMenu() {
        ...
        if let lastLongPressTouhLocation = lastLongPressTouchPoint {
            if configuration.preferences.javaScriptEnabled {
                evaluateJavaScript("window.\(JAVASCRIPT_BRIDGE_NAME)._findElementsAtPoint(\(lastLongPressTouhLocation.x),\(lastLongPressTouhLocation.y))", completionHandler: { value, error in
                    if error != nil {
                        print("Long press gesture recognizer error: \(error?.localizedDescription ?? "")")
                    } else if let value = value as? [String: Any?] {
    ...
    override public func observeValue(forKeyPath keyPath: String?, of object: Any?,
        ...
        if keyPath == #keyPath(WKWebView.estimatedProgress) {
            ...
            if keyPath == #keyPath(WKWebView.cameraCaptureState) || keyPath == #keyPath(WKWebView.microphoneCaptureState) {
                var oldState: WKMediaCaptureState?
                if let oldValue = change?[.oldKey] as? Int {
                    oldState = WKMediaCaptureState(rawValue: oldValue)
                }
                var newState: WKMediaCaptureState?
                if let newValue = change?[.newKey] as? Int {
                    newState = WKMediaCaptureState(rawValue: newValue)
                }
...
//            }
    ...
    public func goBackOrForward(steps: Int) {
        if canGoBackOrForward(steps: steps) {
            if steps > 0 {
                let index = steps - 1
                go(to: backForwardList.forwardList[index])
            } else if steps < 0 {
                let backListLength = backForwardList.backList.count
                let index = backListLength + steps
                go(to: backForwardList.backList[index])
            }
        }
    }
    ...
    public func canGoBackOrForward(steps: Int) -> Bool {
        ...
        return (steps >= 0)
            ? steps <= backForwardList.forwardList.count
    ...
    public func takeScreenshot(with: [String: Any?]?, completionHandler: @escaping (_ screenshot: Data?) -> Void) {
        ...
        takeSnapshot(with: snapshotConfiguration, completionHandler: { image, _ in
            var imageData: Data?
    ...
    func setSettings(newSettings: InAppWebViewSettings, newSettingsMap: [String: Any]) {
        ...
        if #available(iOS 11.0, *) {
            ...
            if newSettingsMap["contentInsetAdjustmentBehavior"] != nil && settings?.contentInsetAdjustmentBehavior != newSettings.contentInsetAdjustmentBehavior {
                scrollView.contentInsetAdjustmentBehavior =
                    UIScrollView.ContentInsetAdjustmentBehavior(rawValue: newSettings.contentInsetAdjustmentBehavior)!
            }
        }
        ...
        if newSettingsMap["selectionGranularity"] != nil && settings?.selectionGranularity != newSettings.selectionGranularity {
            configuration.selectionGranularity = WKSelectionGranularity(rawValue: newSettings.selectionGranularity)!
        }
    ...
    func getSettings() -> [String: Any?]? {
        ...
        return settings!.getRealSettings(obj: self)
    }
...
//    }
    ...
    public func callAsyncJavaScript(functionBody: String, arguments: [String: Any], completionHandler: ((Any?) -> Void)? = nil) {
        ...
        evaluateJavaScript(jsToInject) { _, error in
            if let error = error {
                let userInfo = (error as NSError).userInfo
                let errorMessage = userInfo["WKJavaScriptExceptionMessage"] ??
                    userInfo["NSLocalizedDescription"] as? String ??
    ...
    public func webView(_ webView: WKWebView,
        ...
        callback.defaultBehaviour = { (_: PermissionResponse?) in
            if !decisionHandlerCalled {
                decisionHandlerCalled = true
                decisionHandler(.deny)
            }
        }
        callback.error = { [weak callback] (code: String, message: String?, _: Any?) in
            print(code + ", " + (message ?? ""))
    ...
    public func webView(_ webView: WKWebView,
        ...
        self.webView(webView, decidePolicyFor: navigationAction, decisionHandler: { navigationActionPolicy in
    ...
    public func webView(_ webView: WKWebView,
        ...
        callback.defaultBehaviour = { (_: WKNavigationActionPolicy?) in
            if !decisionHandlerCalled {
                decisionHandlerCalled = true
    ...
    public func webView(_ webView: WKWebView,
        ...
        if let response = navigationResponse.response as? HTTPURLResponse, response.statusCode >= 400 {
            let request = WebResourceRequest(fromWKNavigationResponse: navigationResponse)
            let errorResponse = WebResourceResponse(fromWKNavigationResponse: navigationResponse)
        ...
        if useOnNavigationResponse != nil, useOnNavigationResponse! {
            ...
            callback.defaultBehaviour = { (_: WKNavigationResponsePolicy?) in
                if !decisionHandlerCalled {
                    decisionHandlerCalled = true
                    decisionHandler(.allow)
                }
    ...
    public func webView(_ webView: WKWebView, didReceive challenge: URLAuthenticationChallenge, completionHandler: @escaping (URLSession.AuthChallengeDisposition, URLCredential?) -> Void) {
        ...
        if challenge.protectionSpace.authenticationMethod == NSURLAuthenticationMethodHTTPBasic ||
            ...
            callback.defaultBehaviour = { (_: HttpAuthResponse?) in
            ...
            callback.defaultBehaviour = { (_: ServerTrustAuthResponse?) in
                if !completionHandlerCalled {
                    completionHandlerCalled = true
                    completionHandler(.performDefaultHandling, nil)
                }
            ...
            callback.defaultBehaviour = { (_: ClientCertResponse?) in
                if !completionHandlerCalled {
                    completionHandlerCalled = true
                    completionHandler(.performDefaultHandling, nil)
                }
    ...
    public func webView(_ webView: WKWebView, runJavaScriptAlertPanelWithMessage message: String,
        ...
        callback.error = { (code: String, message: String?, _: Any?) in
            if !completionHandlerCalled {
                completionHandlerCalled = true
                print(code + ", " + (message ?? ""))
                completionHandler()
    ...
    func createConfirmDialog(message: String?, responseMessage: String?, confirmButtonTitle: String?, cancelButtonTitle: String?, completionHandler: @escaping (Bool) -> Void) {
        ...
        confirmController.addAction(UIAlertAction(title: okButton, style: .default, handler: { _ in
            completionHandler(true)
        }))

        confirmController.addAction(UIAlertAction(title: cancelButton, style: .cancel, handler: { _ in
    ...
    func createPromptDialog(message: String, defaultValue: String?, responseMessage: String?, confirmButtonTitle: String?, cancelButtonTitle: String?, value: String?, completionHandler: @escaping (String?) -> Void) {
        ...
        promptController.addAction(UIAlertAction(title: okButton, style: .default, handler: { _ in
            if let v = value {
                completionHandler(v)
        ...
        promptController.addAction(UIAlertAction(title: cancelButton, style: .cancel, handler: { _ in
            completionHandler(nil)
        }))
    ...
    public func webView(_ webView: WKWebView,
        ...
        callback.defaultBehaviour = { (_: Bool?) in
...
//    }
        ...
        if let printInfo = printController.printInfo {
            ...
            if let settings = settings {
                if let orientationValue = settings.orientation,
                   let orientation = UIPrintInfo.Orientation(rawValue: orientationValue) {
                    printInfo.orientation = orientation
                }
                if let duplexModeValue = settings.duplexMode,
                   let duplexMode = UIPrintInfo.Duplex(rawValue: duplexModeValue) {
                    printInfo.duplex = duplexMode
                }
                if let outputTypeValue = settings.outputType,
                   let outputType = UIPrintInfo.OutputType(rawValue: outputTypeValue) {
    ...
    public func getHitTestResult(completionHandler: @escaping (HitTestResult) -> Void) {
        if configuration.preferences.javaScriptEnabled, let lastTouchLocation = lastTouchPoint {
            evaluateJavaScript("window.\(JAVASCRIPT_BRIDGE_NAME)._findElementsAtPoint(\(lastTouchLocation.x),\(lastTouchLocation.y))", completionHandler: { value, error in
                if error != nil {
    ...
    public func clearFocus() {
        scrollView.subviews.first?.resignFirstResponder()
    }
    ...
    public func canScrollVertically() -> Bool {
        return scrollView.contentSize.height > frame.height
    }
